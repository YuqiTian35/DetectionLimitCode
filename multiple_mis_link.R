# Table S7: Link function misspecification 
library(tidyverse)
library(rms)
library(kableExtra)
library(multipleDL)
...
for(i in 1:reps){
  ...
  mod <-  multipleDL(y_obs ~ x, data = data.obs, link = 'logit', delta_lower = data.obs$dl_lower)
  # quantiles
  q.est <- quantile_dl(mod, new.data, probs=0.5)
  # cdf
  cdf.est <- cdf_dl(mod, new.data, at.y = 1.5)
  
  beta.est[i] <- mod$coef['x'] * sqrt(3 / pi^2)
  se.est[i] <- sqrt(mod$var['x', 'x']) * sqrt(3 / pi^2)
...
for(i in 1:reps){
  ...
  mod <-  multipleDL(y_obs ~ x, data = data.obs, link = 'loglog', delta_lower = data.obs$dl_lower)
  # quantiles
  q.est <- quantile_dl(mod, new.data, probs=0.5)
  # cdf
  cdf.est <- cdf_dl(mod, new.data, at.y = 1.5)
