# simulation - multiple DLs scenario 3 [Table 2]
# upper DLs at 0.16, 0.30, 0.50
...
set.seed(35)
beta.true <- 1
q50.true <- c(quantile(exp(rnorm(1e5, 0, 1) + 0), 0.03), quantile(exp(rnorm(1e5, 0, 1) + 1), 0.03))
cdf.true <- c(mean(exp(rnorm(1e5, 0, 1) + 0) <= 0.1),
...
func_res <- function(n_each, num_site=3){
  res <- sapply(1:reps, function(i){
    ...
    # data generation
    data <- data_gen(n_each = n_each, num_site = num_site, upper_dl = c(0.16, 0.30, 0.5))
    ...
    q50.res <- quantile_dl(mod, new.data, probs=0.03)
    q50.est <- q50.res[["est"]]
    q50.lb <- q50.res[["lb"]]
    q50.ub <- q50.res[["ub"]]
    ...
    ## conditional cdf
    cdf.res <- cdf_dl(mod, new.data, at.y = 0.1)
    cdf.est <- cdf.res[["est"]] 
    cdf.se <- cdf.res[["se"]]
    cdf.lb <- cdf.res[["lb"]]
    cdf.ub <- cdf.res[["ub"]]
...
func_tab <- function(result1, result2){
  ...
  rownames(tab) <- c("bias of beta",  "percent bias of beta", 
                     ...
                     "bias of Q(0.03|X=0)", "bias of Q(0.03|X=1)",
                     "percent bias of Q(0.03|X=0)", "percentbias of Q(0.03|X=1)",
                     "rmse of Q(0.03|X=0)", "rmse of Q(0.03|X=1)",
                     "ci of Q(0.03|X=0)", "ci of Q(0.03|X=1)",
                     
                     "bias of F(0.1|X=0)", "bias of F(0.1|X=1)",
                     "percent bias of F(0.1|X=0)", "percent bias of F(0.1|X=1)",
                     "empirical estimates of sd(F(0.1|X=0))", "empirical estimates of sd(F(0.1|X=1))",
                     "mean of estimated sd(F(0.1|X=0))", "mean of estimated sd(F(0.1|X=1))",
                     "rmse of F(0.1|X=0)", "rmse of F(0.1|X=1)",
                     "ci of F(0.1|X=0)", "ci of F(0.1|X=1)")
