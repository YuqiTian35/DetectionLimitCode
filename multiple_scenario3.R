...
set.seed(35)
beta.true <- 1
med.true <- c(quantile(exp(rnorm(1e5, 0, 1) + 0), 0.03), quantile(exp(rnorm(1e5, 0, 1) + 1), 0.03))
cdf.true <- c(mean(exp(rnorm(1e5, 0, 1) + 0) <= 0.1),
              mean(exp(rnorm(1e5, 0, 1) + 1) <= 0.1))

# generate the true data and the observed data
data_gen <- function(n_each, num_site, lower_dl = NULL, upper_dl = c(0.16, 0.30, 0.50)){
