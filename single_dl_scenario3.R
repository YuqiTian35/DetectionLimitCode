...
data_one_dl <- function(n, type=c("lower", "upper", "both")){
  ...
  if(type == "lower"){
    dat_dl$y <- ifelse(dat_dl$y < lower_dl, lower_dl, dat_dl$y)
  }else if(type == "upper"){
    dat_dl$y <- ifelse(dat_dl$y > upper_dl, upper_dl, dat_dl$y)
  }else{
    dat_dl$y <- ifelse(dat_dl$y < lower_dl, lower_dl,
                       ifelse(dat_dl$y > upper_dl, upper_dl, dat_dl$y))
  }
...
result_100 <- func_res(n=100, type="upper")
result_500 <- func_res(n=500, type="upper")
